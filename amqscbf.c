...
/*     char Buffer[10];
  ...
  for(int i=0; i< 10000; i++)
  {
   ...
   if (CompCode == MQCC_FAILED)
   {
     ...
     if (OpenCode == MQCC_FAILED)
     {
       printf("MQOPEN2 of '%.48s' ended with reason code %d\n", "TESTQ",Reason);
       goto MOD_EXIT;
     }

    md1.Encoding       = MQENC_NATIVE;
    md1.CodedCharSetId = MQCCSI_Q_MGR;

    gmo.MatchOptions = 0;
    gmo.Options = MQGMO_WAIT           /* wait for new messages       */
               | MQGMO_NO_SYNCPOINT   /* no transaction              */
               | MQGMO_CONVERT;       /* convert if necessary        */
    gmo.WaitInterval = 3000;          /* 15 second limit for waiting */



     MQGET(Hcon,                /* connection handle                 */
           Hobj1,                /* object handle                     */
           &md1,                 /* message descriptor                */
